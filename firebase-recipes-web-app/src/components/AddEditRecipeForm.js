...
if(ingredients.length === 0){
 ...
 const handleAddIngredient = (e) => {
    ...
    return <form className='add-edit-recipe-form-container' onSubmit={handleRecipeFormSubmit}>
<h2>Add New Recipe</h2>
<div className='top-form-section'>
    <div className='fields'>
    <label className='recipe-label input-label'>
        Recipe Name:
        <input type='text' value={name} onChange={(e)=> setName(e.target.value)} className='input-text' />
    </label>
    <label className='recipe-label input-label'>
        Category:
    ...
    <label className='recipe-label input-label'> Publish Date:
...
<div className='ingredient-form'>
    <label className='ingredient-label'>
        Ingredient:
        <input type='text' value={ingredientName} onChange={(e)=> setIngredientName(e.target.value)} onKeyPress={handleAddIngredient} className='input-text' placeholder='ex. 1 cup of sugar' />
    </label>
    <button type='button' className='primary-button add-ingredient-button' onClick={handleAddIngredient}>Add Ingredient</button>
</div>
</div>
<div className='action-buttons'><button type='submit' className='primary-button action-button'>Create Recipe</button></div>
    </form>
}
